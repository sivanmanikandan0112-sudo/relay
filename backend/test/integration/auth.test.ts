...
describe("POST /api/auth/signup", () => {
  it("creates a real COACH account with the caller's own chosen password and logs them straight in", async () => {
    const res = await request(app).post("/api/auth/signup").send({
      username: "signup.newcoach",
      email: "signup.newcoach@example.com",
    ...
    expect(res.body.token).toEqual(expect.any(String));
    expect(res.body.user).toMatchObject({ username: "signup.newcoach", role: "COACH", isSuperAdmin: false, hasCoach: false });

    // The chosen password -- not a generated one -- is what's actually stored: a real login with it works.
    const login = await request(app)
      .post("/api/auth/login")
      .send({ username: "signup.newcoach", password: "MyOwnPassword1!" });
    expect(login.status).toBe(200);

    const stored = await prisma.user.findUniqueOrThrow({ where: { username: "signup.newcoach" } });
    expect(stored.schoolId).toBeNull(); // solo coach until they self-serve a school, same as any other coach
  });

  it("409s a taken username, leaving the existing account untouched", async () => {
    await createCoach({ username: "signup.taken", firstName: "Existing", lastName: "Coach" });
    const res = await request(app).post("/api/auth/signup").send({
      username: "signup.taken",
    ...
    expect(await prisma.user.count({ where: { username: "signup.taken" } })).toBe(1);
  });

  it("409s a taken email", async () => {
    await createCoach({ username: "signup.emailowner", firstName: "Owner", lastName: "Coach" });
    const owner = await prisma.user.findUniqueOrThrow({ where: { username: "signup.emailowner" } });
    const res = await request(app).post("/api/auth/signup").send({
      username: "signup.differentusername",
      email: owner.email,
      password: "SomePassword1!",
      firstName: "New",
      lastName: "Person",
  ...
  it("400s a password under 8 characters", async () => {
    const res = await request(app).post("/api/auth/signup").send({
      username: "signup.shortpw",
      email: "signup.shortpw@example.com",
      password: "short",
      firstName: "New",
      lastName: "Coach",
    });
    expect(res.status).toBe(400);
    expect(await prisma.user.findUnique({ where: { username: "signup.shortpw" } })).toBeNull();
  });

  it("400s an invalid username (matches the same rule invite-accept uses)", async () => {
    const res = await request(app).post("/api/auth/signup").send({
      username: "not a valid username!",
      email: "signup.badusername@example.com",
      password: "SomePassword1!",
      firstName: "New",
      lastName: "Coach",
    });
